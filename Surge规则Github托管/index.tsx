...
import { parsePolicyText, withBuiltInPolicies } from './github-core'
...
function RuleRow({
  ...
  if (rule.type === RAW_TYPE) {
      ...
      return (
        <Button key={rule.id} action={onTap}>
          <HStack spacing={8}>
            ...
            <Text foregroundStyle="secondaryLabel" font="caption" lineLimit={1} truncationMode="middle">{rule.value}</Text>
            <Spacer />
          </HStack>
        </Button>
      )
    ...
    return (
      <Text key={rule.id} foregroundStyle="secondaryLabel" font="caption" lineLimit={1} truncationMode="middle">
        {rule.value || '(空行)'}
      </Text>
    )
  }
  return (
    <Button key={rule.id} action={onTap}>
      <HStack spacing={8}>
        ...
        <Text font="caption" foregroundStyle="secondaryLabel" lineLimit={1}>{rule.type}</Text>
        <Text lineLimit={1} truncationMode="middle">{rule.value}</Text>
        {rule.trailing ? (
          <Text font="caption" foregroundStyle="secondaryLabel" lineLimit={1} truncationMode="middle">{rule.trailing}</Text>
        ) : null}
        <Spacer />
      </HStack>
    </Button>
...
function FullscreenValueEditorView({
  title, initialValue,
}: {
  title: string
  initialValue: string
    ...
    <NavigationStack>
      <VStack
        ...
        toolbar={{
          ...
          topBarTrailing: (
            <Button title="完成" action={() => dismiss(String(controller.content ?? ""))} />
          ),
        }}
      >
        <Editor
          controller={controller}
          scriptName="Surge规则Github托管"
          showAccessoryView
          searchEnabled
...
function RuleEditorView({
  ...
  const policies = withBuiltInPolicies(getConfig().policies)
  ...
  // 全屏编辑「值」（长表达式如 AND/OR 组合更方便）
  async function openFullscreenValueEditor() {
    const next = await Navigation.present<string | null>({
      element: <FullscreenValueEditorView title="编辑值" initialValue={value.value} />,
  ...
  return (
    ...
    >
      ...
      <Section header={<Text>值</Text>} footer={<Text>例如 github.io、1.2.3.4/24；长表达式可点下方按钮全屏编辑</Text>}>
        <TextField
          title="值"
          value={value}
          prompt="规则值"
        ...
        <Button
          ...
          action={openFullscreenValueEditor}
        />
      </Section>
