...
export interface ParsedFile {
  /** 文件头 + [Rule] 段头 + 段内首条可编辑内容前的空行（不可编辑，原样保留） */
  preamble: string[]
  /** [Rule] 块内的可编辑行（规则 / 独立注释 / 空行） */
  rules: Rule[]
  /** [Rule] 块之后的所有行（从下一个 section 头开始，如 [Script]、[Host]），原样保留不解析 */
  postamble: string[]
...
/** 是否 section 头（如 [Rule]、[Script]、[Host]、[URL Rewrite]） */
...
export function parseFile(content: string): ParsedFile {
  if (content === '') {
    return { preamble: [], rules: [], postamble: [], trailingNewline: false }
  }
  const trailingNewline = content.endsWith('\n')
  const lines = content.split('\n')
  if (trailingNewline) lines.pop()

  // 严格限定在 [Rule] 块内读写：仅当文件存在 [Rule] 段头时，把其后的
  // 其它 section（[Script]、[Host] 等）作为 postamble 原样保留、不解析。
  // 无 [Rule] 段头的文件（如纯规则列表）维持旧行为：整个文件视为可编辑块。
  const ruleHeaderIndex = lines.findIndex(line => isRuleHeader(line))
  const blockStart = ruleHeaderIndex >= 0 ? ruleHeaderIndex + 1 : 0
  const nextSectionIndex = ruleHeaderIndex >= 0
    ? lines.findIndex((line, i) => i > ruleHeaderIndex && isSectionHeader(line))
    : -1
  const blockEnd = nextSectionIndex >= 0 ? nextSectionIndex : lines.length

  const initialPreamble = ruleHeaderIndex >= 0 ? lines.slice(0, blockStart) : []
  const postamble = lines.slice(blockEnd)
  const body = lines.slice(blockStart, blockEnd)

  const firstRuleIndex = body.findIndex(line => parseRuleLine(line) !== null)
  const leadingEnd = firstRuleIndex < 0 ? body.length : firstRuleIndex
  const leading = splitLeadingEditableComments(body.slice(0, leadingEnd))
  const preamble = [...initialPreamble, ...leading.preamble]
  const rules: Rule[] = []
  rules.push(...leading.comments.map(line => rawRule(line)))

  if (firstRuleIndex < 0) {
    return { preamble, rules, postamble, trailingNewline }
  }

  for (const line of body.slice(firstRuleIndex)) {
...
export function serializeFile(file: ParsedFile, currentRules: Rule[]): string {
  ...
  const output = [...preamble, ...currentRules.map(formatRule), ...(file.postamble ?? [])]
